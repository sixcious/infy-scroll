...
const Options = (() => {
  ...
  async function init() {
    ...
    // DOM["#database-custom-enable-input"].addEventListener("change", function () {
    //   chrome.storage.local.set({"databaseCustomEnabled": this.checked});
    //   DOM["#database-custom-settings"].className = this.checked ? "display-block fade-in" : "display-none";
    //   // We still want to show the extras (blacklist, whitelist) if databaseCustomEnabled or if the default database (AP) is enabled
    //   DOM["#database-options"].style.display = this.checked || (items.databaseAP && items.databaseAP.length > 0) ? "block" : "none";
    // });
    // DOM["#database-custom-options-enable-input"].addEventListener("change", function () { chrome.storage.local.set({"databaseCustomOptions": this.checked}); });
    // DOM["#database-custom-save-button"].addEventListener("click", saveCustomDatabase);
    // Shortcuts
    ...
    DOM["#scroll-detection-radios"].addEventListener("change", function (event) {
      ...
      DOM["#scroll-detection-throttle"].className = event.target.value === "io" ? "display-none" : "row fade-in";
    });
    ...
    DOM["#scroll-loading-input"].addEventListener("change", function () {
      chrome.storage.local.set({"scrollLoading": this.checked});
      // DOM["#scroll-loading-style"].className = this.checked ? "row fade-in" : "display-none";
    });
    // DOM["#scroll-loading-style-radios"].addEventListener("change", function (event) { saveInput(event.target, "scrollLoadingStyle", "value"); });
    ...
    //   MDC.layout();
    //   chrome.storage.local.set({"nextLinkType": value});
    // });
    ...
    //   MDC.layout();
    //   chrome.storage.local.set({"prevLinkType": value});
    // });
    // DOM["#next-selector-input"].addEventListener("input", function () { saveInput(this, "nextLinkSelector", "value"); });
    // DOM["#next-xpath-input"].addEventListener("input", function () { saveInput(this, "nextXpath", "value"); });
    DOM["#next-path-input"].addEventListener("input", function () { saveInput(this, "nextLinkPath", "value"); });
    //DOM["#next-property-input"].addEventListener("input", function () { saveInput(this, "nextLinkProperty", "array-split-period"); });
    ...
    DOM["#prev-path-input"].addEventListener("input", function () { saveInput(this, "prevLinkPath", "value"); });
    //DOM["#prev-property-input"].addEventListener("input", function () { saveInput(this, "prevLinkProperty", "array-split-period"); });
    ...
    DOM["#backup-button"].addEventListener("click", async function () {
      ...
      // const backup = await Promisify.storageGet(undefined, undefined, ["databaseAP", "databaseIS", "databaseCustom"]);
  ...
  async function populateValuesFromStorage(values) {
    ...
    if (values === "all") {
      ...
      // DOM["#database-activation-input"].checked = items.databaseActivation;
      ...
      DOM["#database-blacklist-text-field-container"].className =  items.databaseMode === "blacklist" ? "display-block fade-in" : "display-none";
      // DOM["#database-blacklist-row"].className = items.databaseActivation ? "row" : "display-none";
      DOM["#database-blacklist-textarea"].value = items.databaseBlacklist?.join("\n") || "";
      DOM["#database-whitelist-text-field-container"].className = items.databaseMode === "whitelist" ? "display-block fade-in" : "display-none";
      // DOM["#database-whitelist-row"].className = !items.databaseActivation ? "row" : "display-none";
      DOM["#database-whitelist-textarea"].value = items.databaseWhitelist?.join("\n") || "";
      // DOM["#database-custom-enable-input"].checked = items.databaseCustomEnabled;
      // DOM["#database-custom-options-enable-input"].checked = items.databaseCustomOptions;
      // DOM["#database-custom-settings"].className = items.databaseCustomEnabled ? "display-block" : "display-none";
      // try {
      //   DOM["#database-custom-textarea"].value = items.databaseCustom ? JSON.stringify(items.databaseCustom, null, "  ") : "";
      // } catch (e) {
      //   console.log("populateValuesFromStorage() - error stringifying databaseCustom. Error:");
      //   console.log(e);
      // }
      // Scroll
      ...
      DOM["#scroll-detection-throttle"].className = items.scrollDetection === "io" ? "display-none" : "row";
      ...
      // DOM["#scroll-loading-style"].className = items.scrollLoading ? "row" : "display-none";
      // DOM["#scroll-loading-style-append-input"].checked = items.scrollLoadingStyle === "append";
      // DOM["#scroll-loading-style-fixed-input"].checked = items.scrollLoadingStyle === "fixed";
      // Next Prev
      DOM["#next-path-input"].value = items.nextLinkPath;
      // DOM["#next-selector-text-field"].style.display = items.nextLinkType === "selector" ? "" : "none";
      // DOM["#next-selector-input"].value = items.nextLinkSelector;
      // MDC.selects.get("next-type-select").value = items.nextLinkType;
      // DOM["#next-xpath-text-field"].style.display = items.nextLinkType === "xpath" ? "" : "none";
      // DOM["#next-xpath-input"].value = items.nextXpath;
      //DOM["#next-property-input"].value = items.nextLinkProperty.join(".");
      DOM["#next-keywords-textarea"].value = items.nextLinkKeywords;
      DOM["#prev-path-input"].value = items.prevLinkPath;
      // DOM["#prev-selector-text-field"].style.display = items.prevLinkType === "selector" ? "" : "none";
      // DOM["#prev-selector-input"].value = items.prevSelector;
      // MDC.selects.get("prev-type-select").value = items.prevLinkType;
      // DOM["#prev-xpath-text-field"].style.display = items.prevLinkType === "xpath" ? "" : "none";
      // DOM["#prev-xpath-input"].value = items.prevXpath;
      //DOM["#prev-property-input"].value = items.prevLinkProperty.join(".");
  ...
  function changeToolbarIcon() {
    Promisify.runtimeSendMessage({receiver: "background", greeting: "setIcon", icon: this.value });
  ...
  function buildSavesTable(saves) {
    ...
    DOM["#saves-quantity"].textContent = savesExist ? chrome.i18n.getMessage("saves_quantity") + " " + saves.length : "";
  ...
  function viewDatabase(event) {
    ...
    if (element && element.dataset.id && element.classList.contains("database-dialog-opener")) {
      ...
      if (id === "AP" || id === "IS") {
        const database = items["database" + id];
        if (id === "AP") {
  ...
  // /**
  //  * Saves the user's Custom Database. Called when the Save Custom Database Button is pressed.
  //  *
  //  * @returns {Promise<void>}
  //  */
  // async function saveCustomDatabase() {
  //   console.log("saveCustomDatabase() - saving custom database...");
  //   clearTimeout(timeouts.saveCustomDatabase);
  //   timeouts.saveCustomDatabase = setTimeout(async function () {
  //     try {
  //       let value = DOM["#database-custom-textarea"].value;
  //       if (!value || typeof value !== "string" || value.trim() === "") {
  //         // throw new Error(chrome.i18n.getMessage("database_custom_error_empty"));
  //         value = "[]";
  //       }
  //       // parse() will throw any errors if it isn't valid JSON
  //       let databaseCustom = JSON.parse(value);
  //       if (!databaseCustom || typeof databaseCustom !== "object") {
  //         throw new Error(chrome.i18n.getMessage("database_custom_error_json"));
  //       }
  //       // Filter the database to only records who contain the required properties
  //       databaseCustom = databaseCustom.filter(d => d.data && d.data.url && d.data.nextLink && d.data.pageElement);
  //       // Note: We do not map the custom database like we do with the regular database so the user has all their data as they entered it in storage. We map it at runtime on each page load. We do check now though if the database can be mapped (valid)
  ...
  //       // const databaseValid = databaseCustom.map(d => { const r = { "url": d.data.url, "nextLink": d.data.nextLink, "pageElement": d.data.pageElement }; for (const o of ["insertBefore"]) { if (d.data[o]) { r[o] = d.data[o]; } } return r; });
  //       // Sort the database with the longest URLs first to find the most exact URL match first
  //       databaseCustom.sort((a, b) => (a.data.url.length < b.data.url.length) ? 1 : -1);
  //       // If after all this we don't have a length, there's a problem with the database input
  //       // if (databaseCustom.length <= 0 || databaseValid.length <= 0) {
  //       //   throw new Error(chrome.i18n.getMessage("database_custom_error_filter"));
  //       // }
  //       await Promisify.storageSet({"databaseCustom": databaseCustom});
  //       DOM["#database-custom-error"].className = "display-none";
  //       DOM["#database-custom-success"].className = "display-block fade-in";
  //     } catch (e) {
  //       console.log("saveCustomDatabase() - Error:");
  //       console.log(e);
  //       DOM["#database-custom-success"].className = "display-none";
  //       DOM["#database-custom-error"].className = "display-block fade-in";
  //       DOM["#database-custom-error-text"].textContent = e.message;
  //     }
  //   }, 200);
  // }
  ...
  async function downloadBackup() {
    ...
    // Don't backup the databases due to size
    // const storageBackup = await Promisify.storageGet(undefined, undefined, ["databaseAP", "databaseIS", "databaseCustom"]);
    ...
    a.download = (chrome.runtime.getManifest().name + " Backup " + (date ? date : "")).replaceAll(/[\s.:]/g, "_") + ".txt";
  ...
  function resetOptions() {
    ...
    timeouts.resetOptions = setTimeout(async function () {
      ...
      if (items) {
        ...
        // Don't need this anymore as we are setting this to false by default in SDV and only setting it to true when ext is installed
        // options.firstRun = false;
        options.toolbarIcon = items.toolbarIcon || options.toolbarIcon;
        options.saves = items.saves || options.saves;
        // options.whitelist = items.whitelist || options.whitelist;
        ...
        // options.databaseActivation = !!items.databaseActivation;
        ...
        // options.databaseCustom = items.databaseCustom || options.databaseCustom;
        // options.databaseCustomEnabled = !!items.databaseCustomEnabled;
        // options.databaseCustomOptions = !!items.databaseCustomOptions;
      }
