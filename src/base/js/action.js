...
const Action = (() => {
   ...
   * @param currentDocument (optional) the current document to parse
   * @param iframeDocument  (optional) the live iframe document to parse
   * @param extra           (optional) any extra action parameters that may be needed by the action
   * @public
   */
  async function performAction(action, caller, instance, items, currentDocument, iframeDocument, extra) {
    ...
    currentDocument = currentDocument || Scroll.get().currentDocument;
    iframeDocument = iframeDocument || Scroll.get().iframeDocument;
    ...
    switch (action) {
      ...
      case "next":
      case "prev":
        actionPerformed = next(action, caller, instance, currentDocument, iframeDocument);
        break;
      case "increment":
      case "decrement":
   ...
   * @private
   */
  function next(action, caller, instance, currentDocument, iframeDocument) {
    ...
    const result = Next.findLinkWithInstance(instance, action, [currentDocument, iframeDocument, document]);
