...
/**
 * Increment handles all main increment or decrement functions. This includes finding the selection in a string
...
const Increment = (() => {
  ...
  function findSelection(url, strategy, selectionCustom, previousException) {
    ...
    try {
      ...
      if (strategy === "smart" || strategy === "custom") {
      ...
      if (strategy === "lastnumber" || strategy === "smart" || strategy === "custom") {
        const last = /\d+(?!.*\d+)/.exec(url);
        if (last) { return {selection: last[0], selectionStart: last.index}; }
      }
      if (strategy === "firstnumber" || strategy === "lastnumber" || strategy === "smart" || strategy === "custom") {
    ...
    switch (base) {
      ...
      case "custom":
        ...
        if (selectionCustom === "SELECTION_TOO_LARGE!") {
          ...
          error = "base_custom_invalid_error";
        }
        break;
      // Base 2-36
      default:
        ...
        if (!(base >= 2 && base <= 36)) {
          ...
          error = "selection_notalphanumeric_error";
        } else {
  ...
  function incrementURL(action, instance) {
    ...
    IncrementMulti.multiPost(selectionmod, urlmod, instance);
    instance.url = urlmod;
  ...
  function splitDateParts(str, dateFormat) {
    ...
    if (delimiters !== "") {
      ...
      for (let i = 0, currPos = 0; i < dateFormatParts.length; i++) {
        strParts[i] = str.substring(currPos, currPos + dateFormatParts[i].length);
...
const IncrementArray = (() => {
  ...
  function stepThruURLs(action, instance) {
    ...
    if (instance.scrollEnabled || (instance.downloadEnabled && (action === "increment" || action === "list"))) {
      ...
      // Also, there's only one possible direction and it doesn't need auto handling (simply increment the array position)
      // Note: We only do this in downloadEnabled when the action is increment. We still want to go backwards if the user presses the up button
      // TODO: Refactor this for downloadEnabled without doing it this way so it works this way in increment but doesn't work this way in decrement
      // urlProps = instance.urls[instance.urlsCurrentIndex + 1 < instance.urls.length ? instance.urlsCurrentIndex++ : instance.urls.length - 1];
      // Note: We intentionally first increment the index and then use -1 of it for props.
      // Note: If index is bigger than urls, we intentionally set urlsCurrentIndex to length (not length - 1), so that we know we've already reached the end of the list
      instance.urlsCurrentIndex = instance.urlsCurrentIndex + 1 < instance.urls.length ? instance.urlsCurrentIndex + 1 : instance.urls.length;
      urlProps = instance.urls[instance.urlsCurrentIndex - 1];
    } else if ((action === "increment" && !instance.autoEnabled) || (action === instance.action)) {
  ...
  function precalculateURLs(instance) {
    ...
    if (instance.listEnabled || instance.multiRangeEnabled || instance.toolkitEnabled || instance.shuffleEnabled) {
      // List action is always an increment
  ...
  function buildURLs(instance, action, quantity) {
    ...
    // If Toolkit crawl or links, first include the original URL for completeness and include it in the quantity count
  ...
  function buildMultiRangeURLs(instance, action, urls) {
    ...
    for (let i = 1; i <= instance.multiCount; i++) {
      ...
      IncrementMulti.multiPre(action + i, instance);
      IncrementMulti.multiPost(selectionmod, urlmod, instance);
    ...
    for (let i = 0; i < instance.multi[1].times; i++) {
      ...
      if (i !== instance.multi[1].times - 1) {
        Increment.increment(action + "1", instance);
        urls.push({"urlmod": instance.url, "selectionmod": instance.selection});
      }
    }
