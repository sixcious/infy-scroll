...
const Background = (() => {
  ...
  async function installedListener(details) {
    ...
    if (details.reason === "install") {
      ...
      // Note: When the extension is first installed, we await downloading the database and then send a message to Options to repopulate the Database Options after the Database has been downloaded
  ...
  async function messageListener(request, sender, sendResponse) {
    ...
    switch (request.greeting) {
      ...
      case "getSDV":
        ...
        break;
      // case "getRandom":
      //   response = { randomNumber: Cryptography.randomNumber(), randomString: Cryptography.randomString() };
      //   break;
      case "restoreData":
