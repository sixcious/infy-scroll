...
const Storage = (() => {
  ...
  function getStorageDefaultValues() {
    ...
    _.on = true;
    _.toolbarIcon = preferredColor;
    _.buttonSize = 40;
    _.interfaceTheme = false;
    ...
    return _;
    // _.nextLinkType = "selector";
    // _.prevLinkType = "selector";
    // _.buttonType = "selector";
    // _.pageElementType = "selector";
    // _.lazyLoadSource = "data-src";
    // _.lazyLoadDestination = "src";
    // _.whitelist = [];
    // _.whitelistEnabled = false;
    // _.databaseCustom = [];
    // _.databaseCustomEnabled = false;
    // _.databaseCustomOptions = false;
  }
  ...
  /**
   * Gets this platform's name by examining extension API feature support. This is primarily needed so we can change
   * default settings depending on the platform.
   *
  ...
  async function _08(previousVersion) {
    ...
    await Promisify.storageRemove([
      "buttonMethod", "buttonScrollPixels",
      "nextSelector", "nextXpath", "prevSelector", "prevXpath",
      "buttonRule",
      "nextType", "prevType", "buttonType",
      "nextProperty", "prevProperty",
      "nextKeywords", "prevKeywords",
      "selectionPriority",
    ...
    await Promisify.storageSet({
      ...
      // "scrollLoadingStyle": "append",
      "append": items.scrollAppend || "page",
      "selectionStrategy": items.selectionPriority || "smart",
      "shuffleURLs": 100,
      // "buttonDetection": "auto",
      "buttonPosition": 1000,
      // "pageElementType": items && items.scrollElementType ? items.scrollElementType : "selector",
      ...
      "pathQuote": "single",
      "pathOptimized": true,
      "databaseAP": items.database || [],
      "databaseAPDate": items.databaseDate || null,
      "databaseAPLocation": null,
      "databaseAPEnabled": databaseEnabled,
      "databaseIS": [],
      "databaseISDate": null,
      "databaseISLocation": null,
      "databaseISEnabled": databaseEnabled,
    ...
    // Next Keywords - remove space, hyphen, underscore and lowercase
    ...
    await Promisify.storageSet({"saves": saves});
    // Download new IS database and re-download AP database to new databaseAP key (just to be safe in case rename didn't work)
    if (databaseEnabled) {
      await Database.download();
    }
  }
