...
class Append {
  ...
  static async execute(caller) {
    ...
    // Element Iframe Trim should redirect to iframe; Click Element and AJAX Native to ajax-native; all other append modes remain the same
    const mode =
  ...
  static prepend(caller) {
    console.log("Append.prepend() - caller=" + caller);
  ...
  static async prepareFirstPage(caller) {
    console.log("Append.prepareFirstPage() - caller=" + caller);
    // If iframe but not setting page one to iframe, , set to iframe or page depending on what iframePageOne is. Otherwise, do whatever the append mode is
    // const mode = V.instance.append === "iframe" ? V.instance.iframePageOne ? "iframe" : "page" : V.instance.append;
    const mode = V.instance.append === "iframe" && !V.instance.iframePageOne ? "page" : V.instance.append;
    let observableElement;
    switch (mode) {
      case "page":
        // To decide on the observable element, we choose the first page's element by picking the first one with a height > 0 px. We first look at the body's first-level children and then fall-back to walking through every element in the body
      ...
      case "media":
        ...
        break;
      case "none":
        V.clickElement = Click.findElement(V.instance.clickElementPath, V.instance.clickElementType, document, false).element;
        Append.#appendFinally("none", V.clickElement, "prepareFirstPage");
        break;
    }
  }
  ...
  static appendMessage(message, el) {
    console.log("Append.appendMessage() - message=" + message);
  ...
  static async #appendPage(caller) {
    console.log("Append.appendPage() - caller=" + caller);
  ...
  static async #appendIframe(caller) {
    console.log("Append.appendIframe() - caller=" + caller);
    ...
    if (caller === "prepareFirstPage") {
      ...
      document.querySelectorAll("body > *").forEach(element => { if (element !== V.iframe && element !== V.overlay && element !== V.loading && typeof element.remove === "function") { element.remove(); } });
    }
  ...
  static async #appendElement(caller) {
    console.log("Append.appendElement() - caller=" + caller);
    ...
    if (V.instance.pageElementIframe || V.instance.append === "ajax") {
      ...
      if (!V.iframe) {
        console.log("Append.appendElement() - warning: no iframe, even though this V.instance uses iframes. creating iframe...");
    ...
    if (!V.insertionPoint || !V.insertionPoint.parentNode || V.insertionPoint.ownerDocument !== document) {
      console.log("Append.appendElement() - the insertion point's hierarchy in the DOM was altered. " + (V.insertionPoint ? ("parentNode=" + V.insertionPoint.parentNode + ", ownerDocument === document=" + (V.insertionPoint.ownerDocument === document)) : "insertionPoint is undefined!"));
      V.insertionPoint = Elementify.getInsertionPoint(Elementify.getPageElements(document), false);
    }
    DOMNode.insertBefore(fragment, V.insertionPoint);
    // pageElements.forEach(el =>  console.log("Append.adoptNode() - after insertion, ownerDocument === document=" + (el.ownerDocument === document)));
    ...
    if (!observableElement) {
      console.log("Append.appendElement() - no observable element found, manually creating a span");
      observableElement = document.createElement("span");
  ...
  static async #appendMedia(caller) {
    console.log("Append.appendMedia() - mediaType=" + V.instance.mediaType);
    const media = document.createElement("div");
    media.style = Util.processStyle(V.MEDIA_STYLE);
  ...
  static async #appendNone(caller) {
    console.log("Append.appendNone()");
    V.clickElement = Click.findElement(V.instance.clickElementPath, V.instance.clickElementType, document, false).element;
    Append.#appendFinally("none", V.clickElement, caller);
  }
  ...
  static async #appendAjax(caller) {
    ...
    let observableElement = Append.#getObservableElements(V.pageElements)[0];
    if (!observableElement) {
      console.log("Append.appendAjax() - no observable element found, manually creating a span");
      observableElement = document.createElement("span");
      DOMNode.insertBefore(observableElement, V.insertionPoint);
      V.pageElements.unshift(observableElement);
    }
    ...
    for (let i = 1; i <= 3; i++) {
      setTimeout(() => {
        console.log("Append.appendAjax() - setTimeout(), i=" + i);
  ...
  static #appendFinally(mode, observableElement, caller) {
    console.log("Append.appendFinally() - mode=" + mode + ", observableElement=" + observableElement + ", caller=" + caller);
    ...
    if (!observableElement && mode !== "none" && !V.instance.workflowPrepend) {
      console.log("Append.appendFinally() - no el, so removing last divider");
    ...
    // TODO: linksNewTabOneEnabled is a hidden setting; not sure if it is a good idea for page 1 links to open in a new tab. Based on testing, I found it weird on some pages that it did it
    // Note for iframe: If iframePageOne, V.pageElements should be empty, so we don't need to worry about checking for that
    if (caller === "prepareFirstPage" && V.instance.linksNewTabOneEnabled && ["page", "iframe", "element", "ajax"].equals(V.instance.append)) {
      // Some websites dynamic content anchors haven't yet loaded; this is a bit hacky as we are using setTimeout()...
      for (let timeoutCheck = 0; timeoutCheck <= 2000; timeoutCheck += 1000) {
        setTimeout(() => { Append.#setLinksNewTab(V.pageElements); }, timeoutCheck);
      }
    }
    ...
    if (V.pageElements) {
      ...
      // console.log("Append.multiplePageElements, observableMultiplePageElements, page.bottomElements=");
    ...
    if (V.loading && V.instance.showLoading) {
    ...
    if (V.items.statsEnabled) {
      try {
        ...
        console.log("Append.appendFinally() - Error calculating and saving stats, Error:");
        console.log(e);
      }
    }
  }
  ...
  static #appendLoading() {
    console.log("Append.appendLoading() - showLoading=" + V.instance.showLoading);
    try {
      if (V.instance.showLoading && (!V.instance.showIcon || !V.divider)) {
        V.loading.style.setProperty("display", "block", "important");
      }
    } catch (e) {
      console.log("Append.appendLoading() - Error:");
      console.log(e);
    }
  }
   ...
   */
  static appendBottom() {
    // console.log("Append.appendBottom()");
    try {
      // What about none?
      if (V.instance.scrollDetection === "io" && V.instance.append !== "none") {
        if (!V.bottom) {
          ...
          console.log("Append.appendBottom() - creating bottom tag=" + tag + ", bottom.container tag=" + tag2);
      ...
      console.log("Append.appendBottom() - Error:");
      console.log(e);
    }
  }
  ...
  static #appendDivider() {
    // console.log("Append.appendDivider() - V.instance.pageDivider=" + V.instance.pageDivider);
    try {
      ...
      if (V.divider && V.divider.id.endsWith(String(V.instance.pages.length + 1))) {
        console.log("Append.appendDivider() - already appended this page number divider! " + (V.instance.pages.length + 1));
        return;
      }
      if (V.instance.pageDivider === "yes" || (V.instance.pageDivider === "element" && (V.instance.append === "element" || V.instance.append === "ajax"))) {
        ...
        console.log("Append.appendDivider() - divider tag=" + tag + ", divider.container tag=" + tag2);
        // If this is a table row, must calculate colspan before we re-create the divider
        const colSpan = tag === "tr" ? Append.#calculateColSpan() : undefined;
        ...
        if (V.instance.showIcon) {
          const icon = Util.createIcon("infinity", { color: V.instance.color, animated: V.instance.showLoading });
      ...
      console.log("Append.appendDivider() - Error:");
      console.log(e);
    }
  }
  ...
  static async #getNextDocument() {
    console.log("Append.getNextDocument()");
    let nextDocument;
    try {
      ...
      console.log("Append.getNextDocument() - error fetching next document, giving up. Error:");
      console.log(e);
    ...
    try {
      ...
      console.log("Append.getNextDocument() - error cloning document or removing scripts and styles. Error:");
      console.log(e);
    }
    return nextDocument;
  }
  ...
  static #executeCustomScripts(nextDocument, checkRootDocument) {
    console.log("Append.executeCustomScripts()");
    ...
    try {
      ...
      if (V.instance.script >= 0 && Scripts[V.instance.script] && (checkRootDocument ? Scripts[V.instance.script].rootDocument : true)) {
        console.log("Append.executeCustomScripts() - executing a custom script, script url:" + Scripts[V.instance.script].url);
        Scripts[V.instance.script].fn(nextDocument);
      }
    } catch (e) {
      console.log("Append.executeCustomScripts() - error executing custom script. Error:");
      console.log(e);
    }
  }
  ...
  static #fixLazyLoadingPre(nextDocument) {
    console.log("Append.fixLazyLoadingPre() - lazyLoad=" + V.instance.lazyLoad);
    try {
      V.lazys = new Map();
      ...
      if (V.instance.lazyLoad) {
        if (V.instance.lazyLoad === "manual") {
          ...
          console.log("Append.fixLazyLoadingPre() - lazyManuals.length=" + lazyManuals.length);
          ...
          console.log("Append.fixLazyLoadingPre() - lazyImages.length=" + lazyImages.length);
          for (const lazyImage of lazyImages) {
            try {
              ...
              console.log("Append.fixLazyLoadingPre() - " + lazyImage.attributes.length + " attributes:\n" + Object.values(lazyImage.attributes).map(a => a.name + ":" + a.value));
              ...
              if (loadAttribute) {
                console.log("Append.fixLazyLoadingPre() - setting src to attribute: " + loadAttribute.name + " " + loadAttribute.value);
              ...
              console.log("Append.fixLazyLoadingPre() - error encountered when setting a lazy element");
            }
          }
          // const lazyBackgrounds = nextDocument.querySelectorAll(lazyBackgroundAttributes.map(la => "figure[" + la + "], a[" + la + "], div[" + la + "]").join(", "));
          // console.log("Append.fixLazyLoadingPre() - lazyBackgrounds.length=" + lazyImages.length);
          ...
          // console.log("Append.fixLazyLoadingPre() - lazyVideos.length=" + lazyVideos.length);
      ...
      console.log("Append.fixLazyLoadingPre() - error fixing lazy loading. Error:");
      console.log(e);
    }
  }
  ...
  static #fixLazyLoadingPost() {
    console.log("Append.fixLazyLoadingPost()");
    try {
      if (V.lazys && V.lazys.size > 0 && V.instance.lazyLoad) {
        console.log("Append.fixLazyLoadingPost() - lazys.size=" + V.lazys.size);
        ...
        for (const [attribute, lazy] of V.lazys) {
          ...
          if (/blur\(.*\)/.test(lazyStyle.filter)) {
            console.log("Append.fixLazyLoadingPost() - setting filter: blur to 0 because filter was " + lazyStyle.filter);
          ...
          if (/opacity\(.*\)/.test(lazyStyle.filter)) {
            console.log("Append.fixLazyLoadingPost() - setting filter: opacity to 1 because filter was " + lazyStyle.filter);
          ...
          if (/(0)|(\d?\.\d)/.test(lazyStyle.opacity)) {
            console.log("Append.fixLazyLoadingPost() - setting opacity to 1 because opacity was " + lazyStyle.opacity);
      ...
      console.log("Append.fixLazyLoadingPost() - Error:");
      console.log(e);
    }
  }
  ...
  static #calculateColSpan() {
    console.log("Append.calculateColSpan()");
    let colSpan;
    try {
      ...
      if (V.divider?.children[0]?.colSpan > 0) {
        console.log("Append.calculateColSpan() - colSpan already calculated, using prior divider's colSpan");
        ...
        console.log("Append.calculateColSpan() - calculating colSpan for the first time using the table's rows...");
      ...
      console.log("Append.calculateColSpan() - Error:");
      console.log(e);
    }
    if (!colSpan || colSpan <= 0) {
      console.log("Append.calculateColSpan() - no colSpan found, setting colSpan to 1");
      colSpan = 1;
    }
    console.log("Append.calculateColSpan() - colSpan=" + colSpan);
    return colSpan;
  }
  ...
  //  */
  // static #resetStyling() {
  //   console.log("Append.resetStyling()");
  ...
  //         console.log("Append.resetStyling() - setting html.style.overflow to visible");
  //         html.style.setProperty("overflow", "visible", "important");
  //       }
  //       if (htmlStyle.overflowY === "auto" || htmlStyle.overflowY === "scroll") {
  //         console.log("Append.resetStyling() - setting html.style.overflow-y to visible");
  //         html.style.setProperty("overflow-y", "visible", "important");
  //       }
  //       if (bodyStyle.overflow === "auto" || bodyStyle.overflow === "scroll") {
  //         console.log("Append.resetStyling() - setting body.style.overflow to visible");
  //         body.style.setProperty("overflow", "visible", "important");
  //       }
  //       if (bodyStyle.overflowY === "auto" || bodyStyle.overflowY === "scroll") {
  //         console.log("Append.resetStyling() - setting body.style.overflow-y to visible");
  //         body.style.setProperty("overflow-y", "visible", "important");
  //       }
  //     } catch (e) {
  //       console.log("Append.resetStyling() - Error:");
  //       console.log(e);
  //     }
  //   }, 0);
  ...
  static #setLinksNewTab(els = [document]) {
    console.log("Append.setLinksNewTab() - V.instance.linksNewTabEnabled=" + V.instance.linksNewTabEnabled);
    try {
      if (V.instance.linksNewTabEnabled) {
      ...
      console.log("Append.setLinksNewTab() - Error:");
      console.log(e);
    }
  }
  ...
  static #removePages() {
    ...
    if (V.instance.maximumPages > 0 && !V.instance.autoEnabled && !["none"].includes(V.instance.append)) {
      ...
      // console.log("Append.removePages() - allPageElements=");
      ...
      for (let i = 0; i < V.pages.length - V.instance.maximumPages; i++) {
        ...
        console.log("Append.removePages() - isInView=" + isInView + ", elements=");
        ...
        if (!isInView) {
          if (V.instance.append === "element") {
            ...
            console.log("Append.removePages() - height=" + height + ", scrollY=" + window.scrollY + ", scrollY - height=" + (window.scrollY - height) + ", totalHeight=" + Util.getTotalHeight(document) + ", window.scrollY - height > 0 ?=" + ((window.scrollY - height) > 0));
            ...
            console.log("Append.removePages() - moved=" + moved + ", visibleElements=");
  ...
  static #resizeMedia(mode, container, elements) {
    console.log("Append.resizeMedia() - mode=" + mode + ", container=" + container + ", container.children.length=" + (container && container.children ? container.children.length : "") + "elements.length=" + (elements ? elements.length : ""));
    try {
      switch(mode) {
        ...
        case "iframe":
          ...
          console.log("Append.resizeMedia() - windowWidth=" + windowWidth);
          ...
          elements.forEach(el => {
            ...
            if (elementWidth >= windowWidth) {
              console.log("Append.resizeMedia() - resizing el because its width=" + elementWidth + ". Its maxWidth=" + computedStyle.maxWidth);
      ...
      console.log("Append.resizeMedia() - Error:");
      console.log(e);
    }
  }
  ...
  static #getObservableElements(elements) {
    console.log("Append.getObservableElements() - elements.length=" + elements.length);
