 ...
 * 1. Rule - Using a CSS Selector or XPath expression to find the link directly
...
const Next = (() => {
   ...
   *
   * @param type                 the rule type to use ("selector" or "xpath")
   ...
   * @public
   */
  function findLink(type, path, property = ["href"], keywordsEnabled, keywords, keywordObject, checkOtherKeywords, highlight = false, document_ = document) {
    console.log("findLink() - type=" + type + ", rule=" + path + ", property=" + property + ", keywordsEnabled=" + keywordsEnabled + ", keywords=" + keywords + ", keywordObject=" + keywordObject + ", document=" + (document_ ? document_.location : "") + ", highlight=" + highlight);
    ...
    checkRule(type, path, property, document_, details);
    // If a URL was found using the selector or xpath rule, return it (minus the element)
    if (urls.rule) {
      console.log("findLink() - found a URL using the " + urls.rule.method + " rule " + urls.rule.rule + ": " + urls.rule.url);
      highlightElement(urls.rule.element, highlight);
    ...
    if (keywords && (keywordsEnabled || keywordsEnabled === undefined)) {
      checkKeywords(keywords, document_, details);
      console.log("findLink() - found the following next/prev URLs via keywords (no rule match):");
      console.log(urls);
      if (keywordObject) {
   ...
   *
   * @param type      the rule type to use ("selector" or "xpath")
   * @param path      the css selector or xpath rule to use
   * @param property  the array of properties to use e.g. ["href"] or ["parentNode", "href"] for nested properties like parentNode.href
   ...
   * @private
   */
  function checkRule(type, path, property, document_, details) {
    try {
      let element;
      let defaultProperty;
      if (type === "xpath") {
        element = document_.evaluate(path, document_, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      } else {
        element = document_.querySelector(path);
      }
      ...
      if (url && !defaultProperty && property && !property.includes("href")) {
        ...
        try {
          ...
          console.log("checkRule() - Exception caught trying to get the absolute URL from the potential relative URL. Error:")
          console.log(e);
      ...
      if (Util.isValidURL(url, "next-prev", details)) {
        ...
        urls.rule = { "url": url, "method": type, "rule": path + "." + property.join(".") + (defaultProperty ? " (Using the Default Property)" : ""), "element": element };
      }
    } catch (e) {
      console.log("checkRule() - Exception caught when querying for selector or evaluating xpath. Error:");
      console.log(e);
      details.error = e.message;
    }
  }
   ...
   * @private
   */
  function checkKeywords(keywords, document_, details) {
